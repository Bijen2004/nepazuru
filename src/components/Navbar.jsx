    ...
    <nav className="flex flex-row-reverse md:flex-row justify-between items-center h-20 px-6 bg-[#041625] text-white shadow-md">
      ...
      <ul className="hidden md:flex gap-10 text-xl">
        {["Game", "Category", "Leaderboard", "Story", "Gallery"].map((item) => (
          <li key={item} className="cursor-pointer hover:text-gray-300 transition">{item}</li>
        ))}
      </ul>
      ...
      {/* Login Popup */}
      {showLogin && (
        <div className="fixed top-20 right-10 w-96 p-6 bg-white text-black rounded-lg shadow-lg z-50">
          ...
          <div className="mt-4 space-y-3">
            <button className="w-full flex items-center gap-2 p-2 border rounded">
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} className="w-full p-2 border rounded" placeholder="Password" />
              <button
                type="button"
      ...
      {/* Mobile Menu Button */}
      <button onClick={toggleNav} className="block md:hidden" aria-label="Toggle Menu">
        {nav ? <AiOutlineClose size={24} /> : <AiOutlineMenu size={24} />}
      </button>

      {/* Mobile Menu */}
      <div className={`fixed z-20 top-0 left-0 w-3/5 h-full bg-[#041625] border-r border-gray-900 transition-transform ${nav ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <ul className="text-lg p-4 space-y-4">
          {["Game", "Category", "Leaderboard", "Story", "Gallery"].map((item) => (
            <li key={item} className="p-4 border-b border-gray-600 cursor-pointer" onClick={toggleNav}>
              {item}
            </li>
          ))}
