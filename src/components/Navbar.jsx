...
import { FaUserCircle, FaHome, FaLayerGroup, FaTrophy, FaImages } from "react-icons/fa";
...
const Navbar = () => {
  ...
  return (
    <nav className="flex justify-between items-center h-20 px-6 bg-[#041625] text-white shadow-md">
      <Link href="/home">
        <img src="/assets/Nepazuru-logo.png" alt="NePazuru Logo" className="w-32 p-2 cursor-pointer" />
      </Link>

      {/* Desktop Menu */}
      <ul className="hidden md:flex gap-10 text-xl">
        {menuItems.map(({ name, link }) => (
          <li key={name} className="cursor-pointer hover:text-gray-300 transition">
      ...
      {showLogin && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50" onClick={toggleLogin}>
          <div className="bg-[#041625] p-6 rounded-lg text-white" onClick={(e) => e.stopPropagation()}>
      ...
      {showRegister && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50" onClick={toggleRegister}>
          <div className="bg-[#041625] p-6 rounded-lg text-white" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          >
            ...
            <div className="flex justify-between items-center p-4 border-b border-gray-800">
              <Link href="/home">
                <img src="/assets/Nepazuru-logo.png" alt="NePazuru Logo" className="w-32 cursor-pointer" />
              </Link>
            ...
            <ul className="p-4 space-y-4">
              {menuItems.map(({ name, icon, link }) => (
                <li key={name} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-800 cursor-pointer">
            ...
            {/* Sidebar Avatar */}
            <div className="absolute bottom-4 left-4 flex items-center space-x-3 p-4 rounded-lg bg-gray-800 w-11/12 cursor-pointer" onClick={toggleLogin}>
